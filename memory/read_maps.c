...
void share_code_segment()
{
	...
	for(idx=0; idx<mapsRowNum; idx++){
		...
		if(currentRow->needShared){//share this segment
			...
			ADDR original_size = currentRow->end - original_start;
			INT32 original_prot = calculate_mmap_prot(currentRow);
			//change prot to readable
			int ret = mprotect((void*)original_start, original_size, PROT_READ|PROT_WRITE);
			ASSERT(ret==0);
			//copy twice
			memcpy(buf, (const void*)original_start, original_size);
			munmap((void *)original_start, original_size);
			void * map_start = mmap((void*)original_start, original_size, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_FIXED|MAP_ANON, -1, 0);
